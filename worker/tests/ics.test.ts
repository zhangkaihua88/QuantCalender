...
describe('buildCalendarIcs', () => {
  it('creates a stable recurring calendar event with escaped Chinese text', () => {
    ...
    expect(output).toContain('DESCRIPTION:WQ Calendar 会议提醒')
    expect(output).toContain('PRODID:-//WQ Calendar//ZH-CN//EN')
    expect(output).toContain('X-WR-CALNAME:WQ Calendar')
