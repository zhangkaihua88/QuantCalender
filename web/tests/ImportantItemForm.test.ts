...
describe('ImportantItemForm', () => {
  ...
  it('shows bonus milestone fields to administrators and preserves markdown on submit', async () => {
    ...
    expect(wrapper.text()).toContain('周期开始')
    expect(wrapper.text()).toContain('周期结束')
    ...
    await wrapper.find('#important-payment').setValue('2026-12-31')
    await wrapper.find('#important-content').setValue('## 说明\n\n**预计日期**')
    await wrapper.find('form').trigger('submit')
    const submitted = wrapper.emitted('submit')?.[0]?.[0] as { kind:string; contentMarkdown:string; announcementDate:string; startDate:string; endDate:string }
    ...
    expect(submitted.startDate).toBe('2026-07-01')
    expect(submitted.endDate).toBe('2026-09-30')
  })
})
