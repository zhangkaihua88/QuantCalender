...
<template>
  <div class="segmented leaderboard-switch" aria-label="切换排行榜"><button :class="{active:kind==='meeting'}" @click="switchKind('meeting')">会议榜</button><button :class="{active:kind==='replay'}" @click="switchKind('replay')">回放榜</button></div>
  ...
  <div v-else class="stack">
    <div class="metric-grid leaderboard-metrics">
      ...
      <div class="card metric-card"><span>{{ kind === 'meeting' ? '会议投稿' : '投稿链接' }}</span><strong>{{ summary.submissionCount }}</strong></div>
      <div class="card metric-card"><span>{{ kind === 'meeting' ? '通过会议' : '通过链接' }}</span><strong>{{ summary.approvedCount }}</strong></div>
      <div class="card metric-card"><span>{{ kind === 'meeting' ? '整体通过率' : '贡献会议' }}</span><strong>{{ kind === 'meeting' ? `${summary.approvalRate}%` : (summary.contributedMeetingCount || 0) }}</strong></div>
    </div>
    ...
    <div v-if="podium.length" class="leaderboard-podium">
      <article v-for="entry in podium" :key="entry.memberId" class="card podium-card" :class="[`place-${entry.rank}`, { 'is-current':entry.isCurrentUser }]">
        ...
        <span>{{ entry.country }} · {{ kind === 'meeting' ? `投稿 ${entry.submissionCount} 次` : `投稿 ${entry.submissionCount} 个链接` }}</span>
        <b>{{ kind === 'meeting' ? `通过 ${entry.approvedCount} 次` : `贡献 ${replayContribution(entry)} 场会议` }}</b>
      </article>
    </div>

    <div class="card card-body">
      <div class="section-title leaderboard-head"><div><h2>{{ kind === 'meeting' ? '会议投稿排行榜' : '回放贡献排行榜' }}</h2><p class="fine-print">{{ kind === 'meeting' ? '按通过次数排序，其次按投稿次数排序；会议取消后仍保留历史通过记录。' : '按贡献会议数、通过链接数、投稿链接数依次排序；同一成员同场会议最多计 1 次贡献。' }}</p></div><select v-model.number="pageSize" aria-label="排行榜每页数量" @change="load(1)"><option :value="25">每页 25 名</option><option :value="50">每页 50 名</option><option :value="100">每页 100 名</option></select></div>
      <table class="data-table leaderboard-table"><thead><tr><th>排名</th><th>WQ_ID</th><th>地区</th><th>{{ kind === 'meeting' ? '投稿次数' : '投稿链接' }}</th><th>{{ kind === 'meeting' ? '通过次数' : '通过链接' }}</th><th v-if="kind==='replay'">贡献会议</th><th>通过率</th></tr></thead><tbody><tr v-for="entry in entries" :key="entry.memberId" :class="{ 'current-user-row':entry.isCurrentUser }"><td><strong class="rank-number">{{ entry.rank }}</strong></td><td><strong>{{ entry.wqId }}</strong><span v-if="entry.isCurrentUser" class="tag">我</span><br><span v-if="!entry.hasFullWqId" class="fine-print">已隐藏完整 ID</span></td><td>{{ entry.country }}</td><td>{{ entry.submissionCount }}</td><td><strong>{{ entry.approvedCount }}</strong></td><td v-if="kind==='replay'"><strong>{{ replayContribution(entry) }}</strong></td><td>{{ entry.approvalRate }}%</td></tr></tbody></table>
      <div v-if="!entries.length" class="empty-state">目前还没有{{ kind === 'meeting' ? '会议' : '回放' }}投稿。</div>
